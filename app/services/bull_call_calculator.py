...
def calculate_bull_call_profit_and_loss(start_date, short_call_contract, long_call_contract):
    ...
    maximum_profit = (short_call["strike"] - long_call["strike"])*100 - premium_received

    current_profit_loss = calculate_profit_loss(stock_data, premium_received, short_call, long_call)

    break_even_point = calculate_break_even_points(premium_received, long_call)
...
def calculate_premium_received(short_call, long_call):
    premium = abs(short_call - long_call)
    return premium*100
...
def calculate_profit_loss(stock_price, premium, short_call, long_call):
    ...
    if stock_price <= long_call["strike"]:
        ...
        profit = ((stock_price - long_call["strike"]) - (stock_price - short_call["strike"]))*100 - premium
    else:
        profit = (stock_price - long_call["strike"])*100 - premium
    return profit


def calculate_break_even_points(premium, long_call):
    return long_call["strike"] + premium/100


# calculate_bull_call_profit_and_loss("19/07/2023", "AAPL230804C00200000", "AAPL230804C00205000")
